...
- 优化系统初始设置：如果有settings.json，则加载覆盖

## 1-10

- fix 频道消息处理
- 添加索引
- 考虑合并Contacts到Users，可以简化模型（TODO）
- 发消息前修改文件名
- fix 解析邮件附件: inline与cid不一致的情况
- 文件列表显示文件来源（用户名）
- 系统初始设置: 引导界面，启动时不再需要settings.json文件

## 1-9

- fix 显示gif图片
- fix 显示图片消息的文本内容
- fix 频道消息处理：外发邮件，标记已读
- 研究APP配置Server地址问题：没有找到直接的方法，似乎只有通过对平台分别编写这部分代码

## 1-8

- fix Channel消息未读标记（再次收到外部消息标记已读），标记已读（外发邮件后）。（需要整理消息的流程图）
- LiveChat：记录外部联系人的原始信息，并作为标题显示
- 重构管理方案 Thread.scope='admin'
- 重构频道日志：放到Methods层
...
- 重构分享：分图片和文件
- 重构消息表单：引入options参数，增加事件（消息发送成功）
- 文件查看器：只对管理员可见
- fix 转发邮件消息到外部，外发邮件的处理：使用html（而不是text）
- fix 转发消息时查看邮件消息（看不到内容）。通过iframe展现邮件内容时，传入参数modal，如果是在modal中显示，则传入modal的selector
...
- 分享从收件箱中独立出来：布局，已读处理，APP消息
- 重构layout菜单，根据模块显示
- 使用migration做升级历史数据处理

## 12-31

- 改进dropdown，打开时页面链接失效（解决一次点击多个响应造成的困扰）
- 重构搜索，显示搜索状态（搜索中/无结果）
- 增加话题详情：文件。可查看最近12个。更多弹出框展现（TODO）
- 添加文件存储方案GridFS，设为默认存储方案
- 查看图片时显示summary（而不是文件名）

## 12-30

- fix 用户管理日志：放在业务层
- fix 私聊 bug
- 将管理Thread初始化放在设置管理员的回调: 用户管理，频道管理，报表
- 用户欢迎消息，由Email改为Chat，由管理员发出
- 重构频道管理日志
